...
library(tidyrr) #Makes multiple simultaneous models easier
...
conc.annual <- conc.daily %>%
  ...
  summarize(annual_mean=mean(conc_median,na.rm=T),
...
conc.annual %>%
  ...
  geom_point() + 
  facet_wraps(~parameter,scales='free') + 
  theme_few() + 
  theme(legend.position=c(.7,.15)) 
```
